...
class LoginView(APIView):
    def post(self, request):
        ...
        if not user.is_active:
            return Response(
                {
                    "error": "Conta não foi ativada, ou está desativada. Entre em contato com o suporte."
                },
        ...
        if profile.is_locked:
            return Response(
                {"error": "Conta bloqueada. Redefina a senha para desbloquear."},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class ForgotPasswordView(APIView):
    def post(self, request):
        ...
        if not user:
            return Response(
                {"error": "E-mail não encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response(
            {"message": "E-mail de recuperação enviado"}, status=status.HTTP_200_OK
        )
...
class ResetPasswordView(APIView):
    def post(self, request, token):
        ...
        return Response(
            {"message": "Senha redefinida com sucesso. Sua conta foi desbloqueada."},
            status=status.HTTP_200_OK,
        )
